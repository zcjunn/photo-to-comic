...
- **绘画动画漫画风格**：借鉴 painterly-frame 的抽象渲染方法，但只控制色彩、形面、笔触、材质和边缘；主格约用 `5–9` 个大色形，小辅助格约用 `3–6` 个，保留三组明度与清晰动漫人物，不锁源图构图、姿势、视线或比例。
...
行为评测位于 [`evals/evals.json`](evals/evals.json)，共 42 个案例，覆盖最低三格、可选且有理由的多机位、零换机位但非裁切叙事、首尾去重、固定 2:3 拼页、道具完成度、歧义物件、Painterly Comic Animation 连续性、身份与表演变量分离、显式平涂/黑白媒介覆盖等情况。
