...
- When the user does not explicitly request monochrome, pencil, watercolor, flat cel, line art, or another medium, use **Painterly Comic Animation** as the stable default. Its primary medium identity is a hand-painted animation comic page: authored colour, interlocking planes, directional marks, shared light, and broken/lost-and-found edges must be visibly present. It is neither photorealistic painterly rendering nor clean digital cel anime. Sequence authorship always outranks single-frame painterly preservation.
- Put that medium identity near the beginning of the production prompt, before the detailed Panel Map. The first style clause must say what is visibly painted and explicitly reject complete uniform outlines, smooth cel bands, vector-clean surfaces, airbrushed gradients, and generic commercial-anime polish. Do not let words such as `anime face`, `structural contour`, `bright`, or `high saturation` establish the rendering prior before the painterly actions are stated.
- Read [references/painterly-comic-adapter.md](references/painterly-comic-adapter.md) and freeze one rendering-only Style Fingerprint and Completion Lock across every panel: exact portrait `2:3` page geometry; area-adaptive interlocking colour masses; three broad value groups; scene-owned colour roles; reconstructed graphic shapes; faceted planes; connected brush fields; shared boundary illumination; material-specific marks; focal/support/context edge hierarchy; sparse broken contour fragments; and identity-faithful stylized-animation facial proportions. Do not inherit the source photo's pixel ratio, crop, headroom, subject placement, horizon, pose, head axis, gaze, expression, or camera as painterly locks.
...
- Build a **Material Paint Proof Map** from the actual source. Name at least four present materials when available and give each one an observable paint behavior—for example broad layered scumble for sky/cloud, grouped directional wedges for foliage, quiet opaque warm/cool planes for skin, tapered ribbon strokes for hair, angular dry-brush fold fans for cloth, granular broken drags for road/stone, and compact reflected planes for metal. Use only materials that are present; never apply one canvas texture to all of them.
- Connect adjacent forms with related brush direction, temperature, value, and shared illumination. Let contours appear only where they clarify overlap, expression, contact, or a key silhouette turn; elsewhere let strokes cross, break, or lose the edge into shared light. Edges are sharpest at the focal owner, controlled in support areas, and incomplete or atmospheric in context.
- Simplify faces into designed painted-animation planes while preserving identity-bearing facial proportions, relative feature spacing, hair mass, and recurring character cues. Head axis, eye-line, gaze, expression, body pose, and hand state are panel-specific performance variables chosen by the storyboard; they must change intentionally rather than drift accidentally. Match character and environment completion; never place a clean cel-anime figure over a photographic, sketchy, or unrelated painterly plate.
- Require three-scale proof: at thumbnail size, irregular interlocking masses and edge rhythm organize the page; at panel scale, faceted turns, connected stroke currents, and shared illumination are visible; at detail scale, at least three source-present materials have clearly different marks and no important form is enclosed by one perfect vector-like outline. If the page could still be accurately described as a clean digital cel-anime page after ignoring its panel layout, the default style has failed.
...
6. Resolve style priority: explicit user preference or medium request first, then the Painterly Comic Animation house default, then current trend evidence only for compatible abstract refinements. Resolve the six style axes and four or more concrete comic transformation operations; write one Style Bible, one Style Fingerprint, one Material Paint Proof Map, and one Completion Lock shared by every panel.
7. Compile one priority-ordered source-aware prompt. Put the hand-painted, non-cel medium identity and the source-specific material paint proof before the detailed Panel Map; then describe every panel’s role, camera decision, action, value/color job, and transition. State the fixed page geometry and the painterly colour-mass, value-group, plane, brush-continuity, shared-light, edge, face-identity, background-completion, border, and anti-filter contracts once for the whole page. If flat cel is explicitly requested, substitute its alternate completion lock.
8. Run the preflight veto: reject any prompt that could return a single frame; has fewer than three visible panels; lacks a specific state change; copies the source pose/composition into every panel; uses fewer than three shot scales; contains only crop/zoom differences; forces arbitrary camera moves with no narrative or aesthetic gain; describes only a photo filter; puts the default style after long panel detail; omits a source-specific Material Paint Proof Map; permits clean digital cel anime, complete uniform outlines, smooth cel bands, vector-clean surfaces, lineart dominance, pasted blocks, global texture, uniform blur, or mixed finish without an explicit user override; omits the exact portrait `2:3` canvas; allows a four-plus-panel single-column/full-width stack; or leaves a selected primary motif without a stable construction key, meaningful reveal, recurrence, and cross-panel detail hierarchy. Then use the real image generation/editing mechanism once with the photo attached through its actual image-input path. Generate one page by default.
9. Inspect source and result at thumbnail, panel, and detail scale. The result must pass world continuity, prop/motif construction and material continuity, camera/action invention, bookend contrast, compact-page geometry, comic-transformation, the three-scale painted-surface proof or explicit alternate-medium completion, cross-panel Style Fingerprint, and authored-sequence gates. Clean cel-anime rendering is an immediate painterly-default failure even when the story and page layout succeed.
