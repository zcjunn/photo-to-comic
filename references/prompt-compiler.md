...
10. Style Bible, Style Fingerprint, and Completion Lock: the six axes, at least four visible comic transformations, contour-to-fill relationship, opaque color coverage, cel-step count, face/hair/skin simplification, background completion, gradient budget, material-local texture, and focal-versus-context treatment, with no artist or franchise names.
...
Style Bible: [line], [value], [color], [shape/anatomy], [texture], [layout rhythm], [primary motif edge/material/detail hierarchy versus context]. Comic transformation operations: [at least four concrete redraw decisions]. Unless the user explicitly requests another medium, use the Finished Luminous Cel Comic fingerprint: medium dark structural outer contours with thinner interior marks; complete opaque color fills throughout every pictorial panel; two to four stable hard or clean-edged cel value groups; gently simplified identity-faithful anime facial/anatomy planes; grouped hair locks; fully colored simplified backgrounds at the same completion family as the character; local material texture only; soft gradients limited to sky, cloud, mist, spray, reflection, or distant depth; medium dark borders and clean light gutters. At thumbnail size, color masses and grouped values must organize the page before thin line detail. Use one consistent character model, environment model, line family, palette logic, cel-step count, border language, and material finish across all panels. Let the source-derived [primary motif] recur with changed meaning while secondary motifs perform separate jobs. Do not render the source as a photo with a color filter, a softly tinted drawing, or a mixture of sketch and rendered panels.
...
- Unless another medium is explicit, does the prompt freeze the Finished Luminous Cel Comic fingerprint and complete opaque fill coverage across both character and environment?
- Are two to four cel value steps, structural-not-dominant linework, grouped hair/skin planes, local texture, and atmosphere-only gradients explicitly locked across panels?
- Would the page still read first as colored shapes at thumbnail size, rather than as a line drawing with color behind it?
